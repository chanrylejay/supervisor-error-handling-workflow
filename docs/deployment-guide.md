...
| ----------------------------------- | -------- | -------------------------------------------------------------------- |
| NODE\_FUNCTION\_ALLOW\_BUILTIN      | Yes      | Enables `require('crypto')` in Code nodes                            |
| N8N\_BLOCK\_ENV\_ACCESS\_IN\_NODE   | Yes      | Enables `$env.VARIABLE` in Code nodes                                |
| N8N\_BASE\_URL                      | Yes      | Base URL for execution/workflow links (e.g. `http://localhost:5678`) |
| HEALTHCHECKS\_PING\_URL             | Yes      | Dead Man's Switch ping URL                                           |
| SUPERVISOR\_WORKFLOW\_ID            | Yes      | Self-loop guard + orphan exclusion                                   |
| HEARTBEAT\_WORKFLOW\_ID             | Yes      | Self-loop guard + orphan exclusion                                   |
| RETENTION\_WORKFLOW\_ID             | Yes      | Self-loop guard + orphan exclusion                                   |
| TELEGRAM\_CHAT\_ID                  | Yes      | All Telegram alerts                                                  |
| CRITICAL\_WORKFLOW\_IDS             | Yes      | Activation monitoring (comma-separated)                              |
| LONG\_RUNNING\_WORKFLOW\_IDS        | Optional | Orphan detection exclusion (comma-separated)                         |
| NEVER\_RETRY\_WORKFLOW\_IDS         | Optional | Auto-retry safety denylist (comma-separated)                         |
| N8N\_CONCURRENCY\_PRODUCTION\_LIMIT | Optional | Limits concurrent executions (default: 3)                            |
...
| Setting  | Value                              |
| -------- | ---------------------------------- |
| Host     | Your Postgres/Neon pooled endpoint |
| Port     | 5432                               |
...
> **Neon tip:** Add `connectionTimeout: 10` in Connection Options for cold-start tolerance.
...
Create a test workflow with `throw new Error('Supervisor smoke test')`. Set its Error Workflow to Supervisor Core. Activate and trigger. Check Telegram for alert with probable cause advice and Supervisor Action line.
...
Let test workflow fire 5+ times (spaced >30s apart). Watch for escalating "Error X of 5" alerts, then circuit-open banner on the 5th error with transition reason and recovery probe estimate.
...
Manually execute Data Retention. Check Telegram for retention report with data span and humanized reason — or verify `retention_completed` event in database if report was suppressed as routine.
...
If `supervisor_events` is empty despite Telegram alerts arriving, check Postgres credentials on all Postgres nodes.
